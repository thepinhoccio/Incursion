...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.incursion.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.incursion.artwork').getAddonInfo('path'), 'resources', 'media', theme)
...
def artwork():
    execute('RunPlugin(plugin://script.incursion.artwork)')
